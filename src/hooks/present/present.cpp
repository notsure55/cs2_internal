...
namespace Present {
    ...
    DXGI_SWAP_CHAIN_DESC swap_chain_desc { NULL };
    ...
    )
    {
        ...
        Menu::render(context, mainRenderTargetView);
    ...
    void setup() {
        ...
        if (!pDirectPresent) {
            std::println("Failed to setup swap chain!");
            return;
        }
        ...
        {
            std::println("Failed to create hook D3D11CreateDeviceAndSwapChain!");
        }
    }
}
